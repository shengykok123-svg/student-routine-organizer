// Dashboard layout and motivation card refinements.
document.addEventListener("DOMContentLoaded", () => {
  ...
  trackerLink.href = window.location.pathname.replace(
    /\/dashboard\/?$/,
    "/money",
  );
  ...
  chart.data.labels = chart.data.labels.map(
    (label, index) =>
      `${label} · RM ${Number(chart.data.datasets[0].data[index] || 0).toFixed(2)}`,
  );
  chart.update();
});

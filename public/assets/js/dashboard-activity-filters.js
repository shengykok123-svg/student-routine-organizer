document.addEventListener('DOMContentLoaded', () => {
    ...
    const syncAll = () => {
        ...
        allInput.indeterminate = !allInput.checked && seriesInputs.some(input => input.checked);
    };
    const updateSeries = input => {
        chart.setDatasetVisibility(Number(input.dataset.datasetIndex), input.checked);
        chart.update();
        syncAll();
    };
    ...
    seriesInputs.forEach(input => input.addEventListener('change', () => updateSeries(input)));
});

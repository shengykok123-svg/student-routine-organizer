...
$authController = new AuthController($users, $auth, $rememberMe, $notifications);
$dashboardController = new DashboardController($auth, $pdo);
$adminController = new AdminController($auth, $users, $pdo);
$exerciseController = new ExerciseController($auth, new \App\Models\Exercise($pdo), new \App\Models\ExerciseAttachment($pdo), new \App\Services\ExerciseValidationService(), new \App\Services\ExerciseDuplicateConfirmationService(), new \App\Services\ExerciseAttachmentService());
$diaryController = new DiaryController($auth, new \App\Models\DiaryEntry($pdo), new \App\Services\DiaryUploadService());
$habitController = new HabitController($auth, new \App\Models\Habit($pdo), new \App\Models\HabitLog($pdo));
$moneyController = new MoneyController($auth, new \App\Models\MoneyRecord($pdo), new \App\Services\MoneyReceiptUploadService());
$accountController = new AccountController($auth, $users, new \App\Models\UserSettings($pdo), new \App\Services\ProfileImageUploadService());
...
$router->post('exercise/attachment/delete', [$exerciseController, 'deleteAttachment']);

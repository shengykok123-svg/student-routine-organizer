...
final class ExerciseDuplicateConfirmationService
{
    public function issue(array $data): string
    {
        ...
        $_SESSION['_exercise_duplicates'][$token] = ['fingerprint' => $this->fingerprint($data), 'expires' => time() + 600];
        return $token;
    }

    public function confirm(mixed $token, array $data): bool
    {
        $item = is_string($token) ? ($_SESSION['_exercise_duplicates'][$token] ?? null) : null;
        unset($_SESSION['_exercise_duplicates'][$token]);
        return is_array($item) && (int) ($item['expires'] ?? 0) >= time() && hash_equals((string) ($item['fingerprint'] ?? ''), $this->fingerprint($data));
    }

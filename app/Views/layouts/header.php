...
<div class="app-shell">
    <aside class="app-sidebar d-none d-lg-flex">
        ...
        <nav class="sidebar-nav" aria-label="Primary navigation">
            ...
            <?php foreach ($navigation as $item): $isActive = isset($item['path']) ? $relativePath === $item['path'] : $activeModule === $item['key']; $icon = $isActive ? ($item['activeIcon'] ?? $item['icon']) : $item['icon']; ?>
...
<?php if ($authenticated): ?>
    <div class="offcanvas offcanvas-start mobile-nav" tabindex="-1" id="mobileNavigation"><div class="offcanvas-header"><a class="brand-lockup" href="<?= View::e($baseUrl) ?>/<?= $homePath ?>" aria-label="Student Routine Organizer dashboard"><img class="brand-logo" src="<?= $imageBase ?>/branding/logo-app-64.png" alt=""><span>SRO</span></a><button type="button" class="btn-close btn-close-white" data-bs-dismiss="offcanvas" aria-label="Close"></button></div><div class="offcanvas-body d-flex flex-column"><nav class="sidebar-nav"><?php if ($isAdmin): ?><div class="sidebar-section-label">Administration</div><?php endif; ?><?php foreach ($navigation as $item): $isActive = isset($item['path']) ? $relativePath === $item['path'] : $activeModule === $item['key']; $icon = $isActive ? ($item['activeIcon'] ?? $item['icon']) : $item['icon']; ?><a class="sidebar-link <?= $isActive ? 'is-active' : '' ?>" href="<?= View::e($baseUrl) ?>/<?= $item['key'] === 'dashboard' ? 'dashboard' : ($item['path'] ?? $item['key']) ?>"><img class="sidebar-icon" src="<?= $imageBase ?>/sidebar/<?= View::e($icon) ?>" alt=""><span><?= View::e($item['label']) ?></span></a><?php endforeach; ?><div class="sidebar-section-label">Account</div><?php foreach ([['profile','Profile','bi-person'],['settings','Settings','bi-gear']] as [$key,$label,$icon]): ?><a class="sidebar-link <?= $activeModule === $key ? 'is-active' : '' ?>" href="<?= View::e($baseUrl) ?>/<?= $key ?>"><i class="bi <?= $icon ?>"></i><span><?= $label ?></span></a><?php endforeach; ?></nav><form class="sidebar-logout" method="post" action="<?= View::e($baseUrl) ?>/logout"><input type="hidden" name="_csrf" value="<?= View::e(Csrf::token()) ?>"><button type="submit"><img class="sidebar-icon" src="<?= $imageBase ?>/sidebar/sidebar-logout-default.png" alt=""><span>Logout</span></button></form></div></div>
<?php endif; ?>

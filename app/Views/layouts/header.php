...
<div class="app-shell">
    ...
    <div class="app-main">
        <header class="app-topbar"><button class="btn menu-toggle d-lg-none" type="button" data-bs-toggle="offcanvas" data-bs-target="#mobileNavigation" aria-label="Open navigation"><i class="bi bi-list"></i></button><div class="topbar-spacer"></div><button class="btn theme-toggle" type="button" data-theme-toggle aria-label="Switch to dark theme" aria-pressed="false"><span class="theme-toggle-track" aria-hidden="true"><i class="bi bi-sun-fill theme-toggle-icon theme-toggle-icon-sun"></i><i class="bi bi-moon-stars-fill theme-toggle-icon theme-toggle-icon-moon"></i><span class="theme-toggle-thumb"></span></span></button><a class="topbar-bell" href="<?= View::e($baseUrl) ?>/notifications" aria-label="Notifications<?= $unreadNotificationCount ? ': ' . View::e($notificationBadge) . ' unread' : '' ?>"><i class="bi bi-bell"></i><?php if ($unreadNotificationCount > 0): ?><span class="notification-count" aria-hidden="true"><?= View::e($notificationBadge) ?></span><?php endif; ?></a><div class="user-summary"><div><strong>Hello, <?= View::e($auth->username()) ?></strong><small><?= View::e($auth->role()) ?></small></div><?php if ($auth->profileImagePath()): ?><img class="user-avatar user-avatar-image" src="<?= View::e($baseUrl) ?>/profile/photo" alt="Profile picture"><?php else: ?><span class="user-avatar"><?= View::e(strtoupper(substr($auth->username(), 0, 1))) ?></span><?php endif; ?></div></header>

...
/** Handles Money Tracker pages, filters, and transaction actions. */
final class MoneyController extends Controller
{
    ...
    public function index(): void
    {
        ...
        $this->view("money/index", [
            ...
            "categorySummary" => $this->records->categorySummary(
                $user,
                $filterData["filters"],
            ),
    ...
    public function store(): void
    {
        ...
        try {
            $validated["data"]["receipt_path"] = $this->receipts->upload(
                $_FILES["receipt"] ?? [],
            );
    ...
    public function update(): void
    {
        ...
        try {
            ...
            $validated["data"]["receipt_path"] =
                $newReceipt ?: ($removeReceipt ? null : $old["receipt_path"]);
            $this->records->update($id, $user, $validated["data"]);
            if (
                ($newReceipt !== null || $removeReceipt) &&
                $old["receipt_path"]
            ) {
    ...
    public function export(): void
    {
        ...
        header(
            'Content-Disposition: attachment; filename="money-transactions.csv"',
        );
    ...
    public function receipt(): void
    {
        ...
        $record = $this->records->findOwned((int) ($_GET["id"] ?? 0), $user);
        $path = $record
            ? $this->receipts->path($record["receipt_path"] ?? null)
            : null;
    ...
    private function renderEditWithInput(
        int $id,
        array $old,
        array $errors,
    ): void {
    ...
    private function validate(array $input): array
    {
        ...
        if (
            !$date ||
            $date->format("Y-m-d") !== $data["transaction_date"] ||

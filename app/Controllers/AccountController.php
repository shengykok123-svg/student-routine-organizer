...
/** Handles student profile and account preference updates. */
final class AccountController extends Controller
{
    public function __construct(
        ...
        private readonly ProfileImageUploadService $profileImages,
    ) {
    }
    ...
    public function updateProfile(): void
    {
        ...
        try {
            $newImage = $this->profileImages->upload(
            ...
            if ($imagePath !== $existing["profile_image_path"]) {
                $this->users->updateProfileImage($id, $imagePath);
            }
            if (
                ($newImage !== null || $removeImage) &&
                $existing["profile_image_path"]
            ) {
                $this->profileImages->remove($existing["profile_image_path"]);
            }
            $this->auth->login($this->users->findById($id));
            Flash::add("success", "Profile updated.");
            $this->redirect("profile");
        } catch (\RuntimeException $e) {
            $this->renderProfileInput($existing, $name, $username, $email, [
                $e->getMessage(),
            ]);
        }
    }
    ...
    public function profileImage(): void
    {
        ...
        $path = $user
            ? $this->profileImages->path($user["profile_image_path"] ?? null)
            : null;
    ...
    public function updateSettings(): void
    {
        ...
        if (
            ...
            !password_verify(
                $current,
                (string) $this->users->findById($id)["password"],
            )
        ) {
        ...
        if ($errors) {
            $this->view("account/settings", [
                "pageTitle" => "Settings",
                "settings" => [
                    "in_app_notifications" => isset(
                        $_POST["in_app_notifications"],
                    ),
                    "email_notifications" => isset(
    ...
    private function profileErrors(
        ...
        if (
            !$errors &&
            $this->users->usernameOrEmailExists($username, $email, $id)
        ) {
    ...
    private function renderProfileInput(
        array $existing,
        string $name,
        string $username,
        string $email,

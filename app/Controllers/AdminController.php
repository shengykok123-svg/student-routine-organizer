...
/** Manages administrator-only user controls. */
final class AdminController extends Controller
{
    ...
    public function index(): void
    {
        ...
        $summary = [];
        foreach (
            ["users", "exercises", "diary_entries", "money_records", "habits"] as $table
        ) {
            $summary[$table] = (int) $this->pdo
                ->query("SELECT COUNT(*) FROM {$table}")
                ->fetchColumn();
        }
        $summary["students"] = (int) $this->pdo
    ...
    private function valid(
        ...
        $e = [];
        if ($d["full_name"] === "" || mb_strlen($d["full_name"]) > 100) {
            $e[] = "Full name is required and must be 100 characters or fewer.";
        }
        if (!preg_match('/^[A-Za-z0-9_]{4,30}$/', $d["username"])) {
            $e[] = "Username must be 4-30 letters, numbers, or underscores.";
        }
        if (!filter_var($d["email"], FILTER_VALIDATE_EMAIL)) {
            $e[] = "Enter a valid email address.";
        }
        if (!in_array($d["role"], ["Student", "Admin"], true)) {
            $e[] = "Choose a valid role.";
        }
        if (
            ($passwordRequired && strlen($d["password"]) < 6) ||
            (!$passwordRequired &&
                $d["password"] !== "" &&
                strlen($d["password"]) < 6)
        ) {
        ...
        if (
            !$e &&
            $this->users->usernameOrEmailExists(
                $d["username"],
                $d["email"],
                $except,
            )
        ) {
            $e[] = "That username or email is already registered.";
        }
        return $d + ["errors" => $e];
    }
}
